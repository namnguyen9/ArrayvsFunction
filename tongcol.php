...
<body>
    <form method="POST">
        <table border="2" style='color:red'>
            ...
            <tr>
                <td>Row:<input type="number" name="sumcol" placeholder="Nhập số cột cần tính tổng"></td>
    ...
    // Tính Tổng cột theo index
    function SumCol($matrix, $col2)
    {
        $sumcol = 0;
        for ($row = 0; $row < count($matrix); $row++) {
            for ($col = 0; $col < count($matrix[$row]); $col++) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $row = (int) $_POST['row'];
        $col = (int) $_POST['col'];
        $col2 = (int) $_POST['sumcol'];
        $flag = false;
        if ($row >= 0 && $col >= 0 && $col2 >= 0 &&  $col2 < $col  && $row != null && $col != null) {
            $matrix = CreateMatrix($row, $col);
            $show = ShowMatrix($matrix);
            $sumcol = SumCol($matrix, $col2);
            echo "Tổng cột = " . $sumcol;
